...
const Login = () => {
    ...
    axios.post('http://localhost:3000/signin', form)
      .then(result => {
        if(result.data.message === "logged in successfully"){
          // {console.log(result);
          // }
          const auth=({ name: result.data.name, email: form.email,  img: result.data.img, auth: true});
  ...
  return (
<>
        <div className="flex h-[calc(100vh-50px)] min-h-full flex-1 flex-col justify-center px-6 lg:px-8 bg-white">
